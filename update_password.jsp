...
<div style ="background-image: url('http://139.59.78.220:8080/documents/d/guest/banner'); background-size: cover; background-repeat: no-repeat; background-position: center; height: 599px; padding-top: 10px;">
<div class="mt-7 sheet sheet-lg" style="width: 440px;">
		...
		<c:choose>
			...
			<c:otherwise>
				<aui:form action='<%= themeDisplay.getPathMain() + "/portal/update_password" %>' method="post" name="fm">
					...
					<aui:button-row>
						<aui:button type="submit" style="background-color: #003b71;"/>

						<c:if test="<%= showCancelButton %>">
							<aui:button href='<%= themeDisplay.getPathMain() + "/portal/logout" %>' type="cancel" style="background-color: #003b71; color: white" />
						</c:if>
					</aui:button-row>
				</aui:form>
			</c:otherwise>
